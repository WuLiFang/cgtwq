...
class FiltersTestCase(TestCase):
    def test_operations(self):
        result = Filter("title", "text") | Filter("data", "test") & Filter(
            "name", "name"
        )
...
def test_from_arbitrary_args():
    result = cgtwq.FilterList.from_arbitrary_args(cgtwq.Field("key1") == "value1")
    assert isinstance(result, FilterList), type(result)
    assert result == [["key1", "=", "value1"]]

    result = cgtwq.FilterList.from_arbitrary_args(
        cgtwq.Field("key1") == "value1", cgtwq.Field("key2") == "value2"
    )
    assert isinstance(result, FilterList), type(result)
